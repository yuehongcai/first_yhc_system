...
class ApproveAsset(object):
    ...
    def asset_upline(self):
        func = getattr(self,'_%s_upline' % self.new_asset.asset_type)  # 严格按照这样方式命名
        ret = func()
        return ret and True
...
class UpdateAsset(object):
    ...
    def _update_manufacturer(self):
        ...
        if m:
            ...
            self.asset.manufacturer = manufacturer_obj  ## 此处的Manufacturer是Asset表的属性
        else:
    ...
    def _update_RAM(self):
        ...
        need_deleted_keys = set(old_rams.dict.keys()) - set(new_rams_dict.keys())
...
class UpdateServer(object):
    def __init__(self,request,asset_obj,asset_data):
        self.request = request
    ...
    def update(self):
        self.asset_obj.ip = self.data['eth0']['ip']
        self.asset_obj.hostname = self.data.get('hostname')
        self.asset_obj.manufacturer = self.data.get('manufacturer')
        self.asset_obj.product_model = self.data.get('product_model')
        self.asset_obj.cpu_all_core = self.data.get('cpu_core_count')
        self.asset_obj.mem_size = self.data.get('memory_size')
        self.asset_obj.disk_size = self.data.get('disk_size')
        self.asset_obj.os_release = self.data.get('os_release')
        self.asset_obj.asset_type = self.data.get('asset_type')
        self.asset_obj.save()

class NewServer(object):
    def __init__(self,request,asset_data):
        self.request = request
        self.data = asset_data
        self.sn = asset_data.get('sn')
        self.add()

    def add(self):
        defaults = {
            'ip': self.data['eth0']['ip'],
            'hostname': self.data.get('hostname'),
            'manufacturer': self.data.get('manufacturer'),
            'product_model': self.data.get('product_model'),
            'cpu_all_core': self.data.get('cpu_core_count'),
            'mem_size': self.data.get('memory_size'),  # 这是针对客户端脚本发过来的字典
            'disk_size': self.data.get('disk_size'),
            'os_release': self.data.get('os_release'),
            'asset_type': self.data.get('asset_type')
        }
        models.CmdbServer.objects.update_or_create(sn=self.sn,defaults=defaults)